class NsfwDetector {
    ...
    async isNsfwBatch(imageUrls) {
        ...
        try {
            ...
            const outputs = await Promise.all(blobUrls.map(blobUrl => classifier(blobUrl, this._nsfwLabels)));
            console.log(outputs);
            const nsfwDetected = outputs.map(output => output.some(result => result.score > this._threshold));
            return nsfwDetected;
        } catch (error) {
    ...
    async _loadAndResizeImage(imageUrl) {
        ...
        offScreenCanvas.width = 224; // Adjust the size as per your requirements
